...
class Thumb {
    ...
    private function capture($cmdv, $source_path, $type) {
        ...
        // if (!file_exists($capture_path) || filemtime($source_path) >= filemtime($capture_path)) {
            ...
            foreach ($cmdv as &$arg) {
                    $arg = str_replace(['[H5AI_SRC]', '[H5AI_DEST]', '[DUR]'], [$source_path, $capture_path, $timestamp], $arg);
                }
            }
            else {
                foreach ($cmdv as &$arg) {
            ...
            Util::write_log(date('Y-m-d H:i:s') . ' for file ' . $source_path);
            Util::write_log('command:' . implode(' ', $cmdv));
            $result = Util::exec_cmdv($cmdv);
            Util::write_log("result:" . $result[0]);
        }
    ...
    private function compute_duration($cmdv, $source_path) {
        ...
        $result = Util::exec_cmdv($cmdv);
        Util::write_log('original duration:' . $result[0] . '|');
        if (empty($result)) throw new Exception("Incorrect video duration returned: \"".$result."\"");
        // Seek at 15% of the total video duration
        $ret = strval(round(((floatval($result) * 15) / 100), 1, PHP_ROUND_HALF_UP));
        Util::write_log('computed duration:' . $ret);
        return $ret;
    }
}
